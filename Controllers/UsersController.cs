...
namespace BusinessTrip.Controllers
{
    public class UsersController : Controller
    {
        ...
        public async Task<IActionResult> CreateUser(CreateUserView model)
        {
                if (ModelState.IsValid)
                {
                    User user = _db.User.FirstOrDefault(u => u.Email == model.Email);
                    if (user == null)
                    {
                        ...
                        user = new User
                        {
                            ...
                            RoleId = _db.Role.Where(r => r.RoleName == "user").First().Id
                        };
                        Role userRole = _db.Role.FirstOrDefault(r => r.RoleName == "user");
                        if (userRole != null)
                            user.Role = userRole;
